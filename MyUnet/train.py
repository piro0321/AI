...
#========================周囲
        ...
        image_1, contours_1, hierarchy_1 = cv2.findContours(thresh_1,cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

        #色塗り
        for i in range(0, len(contours_1)):
            if len(contours_1[i]) > 0 and  cv2.contourArea(contours_1[i]) < 100000:
                    label_preprocess= cv2.drawContours(label_preprocess, contours_1, i,1, thickness=-1)
        ...
        for i in range(0, len(contours)):
            if len(contours[i]) > 0:
                ...
                if cv2.contourArea(contours[i]) >max_area :
                    keep_label = cv2.drawContours(keep_label, contours, i, 1, thickness=-1)
                if  cv2.contourArea(contours[i]) <max_area :
                    label = cv2.drawContours(label, contours, i, 1, thickness=-1)

        check = np.count_nonzero(label[:,:]*img[:,:] >0 )#画素の白い部分の数（腫瘍領域の四角形の面積)
        if check <26000:
...
def Varidation():
    ...
    for i in range(len(cimage)):
        ...
        cv2.imwrite("./result/test/"+filename+"/target/{0:04d}.jpg".format(i) , cimage[i])
        cv2.imwrite("./result/test/"+filename+"/pred/{0:04d}.jpg".format(i) , pred)
        cv2.imwrite("./result/test/"+filename+"/val/{0:04d}.jpg".format(i) , val)
#-
def test_amed():
    """
    """
    ...
    for i in range(len(cimage)):
        ...
        cv2.imwrite("./result/test/"+filename+"/heatmap/{0:04d}.jpg".format(i),heatmap)
        ...
        cv2.imwrite("./result/test/"+filename+"/target/{0:04d}.jpg".format(i) , cimage[i])
        cv2.imwrite("./result/test/"+filename+"/pred/{0:04d}.jpg".format(i) , pred_resize*255)
        cv2.imwrite("./result/test/"+filename+"/val/{0:04d}.jpg".format(i) , val_resize)
